...
to count the numbers that are not multiple of any $m_i$, then subtract from $x$ to count only the multiples. Now, count how many multiples are in the range from $x+1$ to $n-1$ using any other methods.
\end{document}